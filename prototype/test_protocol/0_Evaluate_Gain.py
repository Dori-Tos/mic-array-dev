...
from classes.Filter import Filter, BandPassFilter, SpectralSubtractionFilter
...
def test_continuous_gain(
	...
	Args:
		...
		num_mics: Number of microphones to use for beamforming.
	...
	try:
		while True:
			...
			if block.ndim == 2 and block.shape[1] > 0:
				input_power_avgch = float(np.mean(block ** 2))
				input_rms_avgch = float(np.sqrt(max(input_power_avgch, 0.0)))
			else:
				input_rms_avgch = input_rms
			input_dbfs_avgch = _safe_dbfs(input_rms_avgch)
			...
			if idx <= baseline_samples:
				baseline_input_ch0.append(input_dbfs)
				baseline_input_avgch.append(input_dbfs_avgch)
				baseline_output.append(output_dbfs)

			baseline_output_db = float(np.mean(baseline_output)) if baseline_output else output_dbfs
			baseline_input_db = float(np.mean(baseline_input_ch0)) if baseline_input_ch0 else input_dbfs
			baseline_input_avgch_db = float(np.mean(baseline_input_avgch)) if baseline_input_avgch else input_dbfs_avgch
			output_delta = output_dbfs - baseline_output_db
			estimated_input_gain_db = input_dbfs - baseline_input_db
			estimated_input_gain_avgch_db = input_dbfs_avgch - baseline_input_avgch_db
			...
			rows.append({
				...
				'input_rms_avgch': input_rms_avgch,
				'input_dbfs_avgch': input_dbfs_avgch,
				'estimated_input_gain_db': estimated_input_gain_db,
				'estimated_input_gain_avgch_db': estimated_input_gain_avgch_db,
			...
			print(
				...
				f"In={input_dbfs:7.2f} dBFS ({estimated_input_gain_db:+6.2f} dB) | "
				f"In(avg)={input_dbfs_avgch:7.2f} dBFS ({estimated_input_gain_avgch_db:+6.2f} dB) | "
	...
	y_in_avgch = analysis_df['input_dbfs_avgch'].to_numpy(dtype=float)
	y_transfer = analysis_df['transfer_db'].to_numpy(dtype=float)
	y_input_gain = analysis_df['estimated_input_gain_db'].to_numpy(dtype=float)
	y_input_gain_avgch = analysis_df['estimated_input_gain_avgch_db'].to_numpy(dtype=float)

	out_slope_s, _, out_r2 = _fit_trend(x, y_out)
	in_slope_s, _, in_r2 = _fit_trend(x, y_in)
	in_avgch_slope_s, _, in_avgch_r2 = _fit_trend(x, y_in_avgch)
	transfer_slope_s, _, transfer_r2 = _fit_trend(x, y_transfer)
	input_gain_slope_s, _, input_gain_r2 = _fit_trend(x, y_input_gain)
	input_gain_avgch_slope_s, _, input_gain_avgch_r2 = _fit_trend(x, y_input_gain_avgch)

	out_slope_min = out_slope_s * 60.0
	in_slope_min = in_slope_s * 60.0
	in_avgch_slope_min = in_avgch_slope_s * 60.0
	transfer_slope_min = transfer_slope_s * 60.0
	input_gain_slope_min = input_gain_slope_s * 60.0
	input_gain_avgch_slope_min = input_gain_avgch_slope_s * 60.0
	...
	significant_drop = (input_gain_slope_min <= -0.5 and input_gain_avgch_slope_min <= -0.5)
	...
	baseline_input_db = float(np.mean(baseline_input_ch0)) if baseline_input_ch0 else float(df['input_dbfs'].iloc[0])
	baseline_input_avgch_db = float(np.mean(baseline_input_avgch)) if baseline_input_avgch else float(df['input_dbfs_avgch'].iloc[0])
	...
	print(f"  Baseline input avgch (first {min(baseline_samples, len(df))}): {baseline_input_avgch_db:.2f} dBFS")
	print(f"  Baseline output (first {min(baseline_samples, len(df))}): {baseline_output_db:.2f} dBFS")
	print(f"  Final estimated input gain (ch0): {df['estimated_input_gain_db'].iloc[-1]:+.2f} dB")
	print(f"  Final estimated input gain (avgch): {df['estimated_input_gain_avgch_db'].iloc[-1]:+.2f} dB")
	print(f"  Final output: {df['output_dbfs'].iloc[-1]:.2f} dBFS")
	print(f"  Estimated input gain slope (ch0): {input_gain_slope_min:+.3f} dB/min (R^2={input_gain_r2:.3f})")
	print(f"  Estimated input gain slope (avgch): {input_gain_avgch_slope_min:+.3f} dB/min (R^2={input_gain_avgch_r2:.3f})")
	print(f"  Output slope: {out_slope_min:+.3f} dB/min (R^2={out_r2:.3f})")
	print(f"  Input slope: {in_slope_min:+.3f} dB/min (R^2={in_r2:.3f})")
	print(f"  Input slope avgch: {in_avgch_slope_min:+.3f} dB/min (R^2={in_avgch_r2:.3f})")
	print(f"  Transfer slope (Out-In): {transfer_slope_min:+.3f} dB/min (R^2={transfer_r2:.3f})")
	print(f"  Interpretation: {'CONTINUOUS INPUT GAIN REDUCTION DETECTED' if significant_drop else 'NO STRONG CONTINUOUS INPUT GAIN REDUCTION DETECTED'}")
...
if __name__ == '__main__':
	...
	parser.add_argument('--num-mics', type=int, default=8,
						help='Number of microphones to use in pipeline mode (default: 8)')
	parser.add_argument('--geometry', type=int, default=2,
						help="Geometry selector by XML filename prefix (default: 2). Example: --geometry 2 loads '2_*.xml'.")
